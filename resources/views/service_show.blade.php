...
-->
    ...
    <div class="px-2 py-8 mx-auto max-w-screen-xl">
        <article class="mx-auto w-full max-w-3xl  rounded-2xl ">
            <header class="mb-6">
                <h1 class="text-3xl md:text-4xl font-bold text-blue-900 dark:text-white leading-tight">
                    {{ $service->title }}
                </h1>
            </header>
            ...
            <div class="space-y-4 text-blue-900 dark:text-gray-300">
            ...
            @if ($service->promotions && $service->promotions->count())
                @foreach ($service->promotions as $promotion)
                    <div class="mt-10">
                        <h2 class="text-2xl font-bold text-blue-900 dark:text-white mb-4">🎁 Promo Khusus untuk Anda
                        </h2>
                        ...
                        {{-- Info Promo --}}
                        <h3 class="text-xl font-semibold text-blue-900 dark:text-white">{{ $promotion->title }}</h3>
                        <p class="mt-2 text-blue-900 dark:text-gray-300">{{ $promotion->description }}</p>

                        @if ($promotion->discount_percent)
                            <p class="mt-3 text-blue-900 dark:text-green-400 font-semibold">
            ...
            @if ($testimonial->count())
                <section class="max-w-screen-xl mx-auto px-4 py-12">
                    <h2 class="text-2xl font-bold text-center text-blue-900 dark:text-white mb-10">
                        Apa Kata Klien Kami?
                    </h2>
                                ...
                                <div class="flex items-center mb-4">
                                    ...
                                    @else
                                        <div
                                            ...
                                            <span class="text-xl text-blue-900 dark:text-white font-bold">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-blue-900 dark:text-gray-400">
                                            {{ $item->position ?? '-' }} • {{ $item->company }}
                                        </p>
                                    </div>
                                </div>

                                <p class="text-blue-900 dark:text-gray-300 italic">“{{ $item->message }}”</p>
                            </div>
                        @endforeach
                    </div>
                </section>
